<script>
	...
	let sensorValue = {};
	async function getCurrentRawValue() {
...
{#if setupSession && setupSession.sensor > 0}
	...
	{#if begin == false}
		<p>
			You have chosen to calibrate the sensor in a unevenly shaped tank. This tank shape requires a more accurate measurement of the volume. Here, we record the
			level over the measurement period and translate the result into a uniform percentage level. This way you always know how much liquid is left in the tank.
		</p>
		<p>
			To perform the calibration, please make sure that your vehicle is level. In particular, you will need a water source that provides a steady flow. Typical
			potable water connections will suffice. Make sure that you can start refueling your vehicle as soon as indicated.
		</p>
		<p>
	...
	{:else}
		...
		<p>
			Ensure a steady inflow and wait until the tank has been completely filled. Once the tank has been completely filled. Turn off the water tap and then
