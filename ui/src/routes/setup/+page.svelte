<script>
	...
	if (browser) {
		setupSession = JSON.parse(window.sessionStorage.getItem('setupSession')) ?? {
			sensor: 0,
			type: undefined
		};
	}
...
{#if setupSession}
	...
	<p>We have to calibrate the sensor in order to use it propperly. This usually only needs to be done once, insofar as you are not changing the tank.</p>
	<p>Which sensor do you want to calibrate?</p>
	<p>
	...
	{#if selectedSensor > 0}
		<p>
			Is it a tank that has been formed uniformly so that the water level rises linearly, or does the tank have significant bulges, size changes, or other
			volume changes?
		</p>
		<div class="row">
			<div class="col text-center" on:click={() => (selectedShape = 1)} on:keypress={() => (selectedShape = 1)} role="button">
				<input type="radio" name="shape" value={1} bind:group={selectedShape} /><br />
				<Fa icon={faRectangle} size="10x" />
				<p>uniformly shaped</p>
			</div>
			<div class="col text-center" on:click={() => (selectedShape = 2)} on:keypress={() => (selectedShape = 2)} role="button">
